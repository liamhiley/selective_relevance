...
class GradCam:
	...
	def __call__(self, input, index = None):
		...
		weights = np.mean(grads_val, axis = tuple(range(2,len(grads_val.shape))))[0, :]
		...
		if len(cam.shape) > 2:
			nshape = [16/cam.shape[0],112/cam.shape[1], 112/cam.shape[2]]
		else:
			nshape = [112 / cam.shape[1], 112 / cam.shape[2]]
		cam = zoom(cam, nshape)
		cam = cam - np.min(cam)
		cam = cam / np.max(cam)
		return cam
    ...
    def __init__(self, model, use_cuda):
        ...
        for idx, module in self.model.features._modules.items():
            if module.__class__.__name__ == 'ReLU':
                self.model.features._modules[idx] = GBReLU()
