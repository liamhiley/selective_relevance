...
def _lrp(samples,mdl,target,target_layers="fc",proportion=False):
    ...
    if isinstance(samples,(list,tuple)):
        ...
        grad = torch.autograd.grad(output, samples, out_mask, retain_graph=True)[0]
    return grad, target
...
class SelectiveRelevanceExplainer:
    ...
    def get_exp(
        ...
        if type(samples) == list:
            samples = [sample.to(device).requires_grad_() for sample in samples]
        else:
